...
def best_psnr(img_orig, img_out, max_value):
            ...
            if abs(hei) % 2 == 0 and abs(wid) % 2 == 0:
                if c == 1:
                    ssim_shifts[(hei + SHIFT) * (SHIFT + 1) + wid + SHIFT, 0] \
                        = ssim(tmp_l[:, :], ssim_h[:, :], max_value).mean()
                else:
                    for i in range(c):
                        ssim_shifts[(hei + SHIFT) * (SHIFT + 1) + wid + SHIFT, i] \
                            = ssim(tmp_l[:, :, i], ssim_h[:, :, i], max_value).mean()

    squared_error = np.square(im_shifts / max_value - im_h / max_value)
