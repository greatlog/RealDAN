...
class Degrader(nn.Module):
    def __init__(
        ...
        deg_scale=1, crop_size=None, return_ratio=False, resize_back=True
        ):
        super().__init__()
    ...
    def forward(self, x, src=None):
        ...
        if hasattr(self, "resize_opt"):
            if self.resize_opt["base"] == "tgt":
                ...
                shape = (h, w)
            lr, scale = self.resize(lr, shape=shape)
            outs.update({"scale": scale})
        ...
        if self.resize_back:
            if lr.shape[2] != tgt_h or lr.shape[3] != tgt_w:
                scale_factor = tgt_h / lr.shape[2]
                lr = imresize(lr, scale_factor)
                # lr = F.interpolate(lr, size=(tgt_h, tgt_w), mode="nearest")
...
class RandomBlur(nn.Module):
    def __init__(self, 
        ...
        ksize=21, return_ratio=False, random_disturb=0
    ):
        super().__init__()
    ...
    def create_kernel(self):
        ...
        radius = torch.randint(min_r, max_r+1, (self.batch,)).to(self.device)
        ratio_radius = self.get_ratio([min_r, max_r+1], radius)
        ...
        if self.random_disturb !=0:
            kernel = kernel + kernel * \
                (torch.rand(self.batch, self.ksize, self.ksize) * 2 - 1).to(self.device) * \
                self.random_disturb
...
class RandomNoise:
    ...
    def __call__(self, img):
        ...
        img = img + noise.float()
        img = torch.clamp(img * 255.0, 0, 255) / 255.0
...
@ARCH_REGISTRY.register()
class RandomResize:
    def __init__(self, range_scale, up_prob, down_prob, return_ratio, base):
    ...
    def __call__(self, img, shape):
        rand = random.random()
        if rand < self.up_prob:
            scale = random.random()
            scale = scale * (self.range_scale[1] - 1) + 1
        elif rand > 1 - self.down_prob:
            scale = random.random()
        ...
        base_h, base_w = shape
        img = F.interpolate(img, size=(int(base_h*scale), int(base_w*scale)), mode="nearest")
        
        scale = torch.FloatTensor([scale]).to(img.device).repeat(img.shape[0])
        ratio = (scale - self.range_scale[0]) / (self.range_scale[1] - self.range_scale[0])
