...
def validate(
    ...
    for (
        ...
        message += "Scores - "
        scores = measure(res=sr_img, ref=gt_img, metrics=eval_opt["metrics"])
        for k, v in scores.items():
            test_results[k][idx] = v
            message += "{}: {:.6f}; ".format(k, v)

        if sr_img.shape[2] == 3 and eval_opt["y_channel"]:  # RGB image
            ...
            message += "Y Scores - "
            scores = measure(res=sr_img_y, ref=gt_img_y, metrics=eval_opt["metrics"])
            for k, v in scores.items():
